<template>
  <div>
    <div class="d-flex">
      <h6 class="text-muted flex-grow-1">KELOLA USER</h6>
    ...
    >
      ...
      <el-table-column label="Role" prop="role_name" :filters="filterRole" column-key="role"></el-table-column>
      <el-table-column
        label="Perusahaan"
        prop="perusahaan"
    ...
    <FormUser
      ...
      :show="showForm"
      @refresh="requestData"
      @close="() => { showForm = false; requestData(); }"
    />
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState(['statusFilter', 'filterPerusahaan', 'filterSite', 'filterRole'])
  },
  methods: {
    requestData() {
      const params = {
      ...
      this.$axios.get('/api/user', { params: Object.assign(params, this.filters) }).then(r => {
        this.tableData = r.data
      })
    },
    deleteData(id) {
      this.$confirm('Anda yakin akan menghapus data ini?', 'Peringatan', { type: 'warning'}).then(() => {
        this.$axios.delete(`/api/user/${id}`).then(r => {
          this.$message({
